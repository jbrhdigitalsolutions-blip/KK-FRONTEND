...
for(const marker of ["analyzeProjectContext","integrationSupportFiles","newProjectSupportFiles","kk-project-fit/v1","APPLY-WINDOWS.ps1","APPLY-MAC.command"]) assert.ok(projectFit.includes(marker),`project-fit marker missing: ${marker}`);
...
for(const marker of ["familyFilters","selectFiltered","Essential design","Build the page yourself.","buildPageButton","downloadProjectButton","authToggle","authMode","Session cookie","projectFitTitle","projectFiles","projectFolder","Analyze Project Fit"]) assert.ok(designExplorer.includes(marker),`Design Explorer marker missing: ${marker}`);
