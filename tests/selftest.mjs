...
assert.ok(fs.existsSync(path.join(CONFIG.packageRoot,"src","reference-design","auth.mjs")));
const referenceAuth=fs.readFileSync(path.join(CONFIG.packageRoot,"src","reference-design","auth.mjs"),"utf8");
...
assert.ok(fs.existsSync(path.join(CONFIG.packageRoot,"src","no-code","project-fit.mjs")));
assert.ok(fs.existsSync(path.join(CONFIG.packageRoot,"src","no-code","project-sources.mjs")));
const projectSources=fs.readFileSync(path.join(CONFIG.packageRoot,"src","no-code","project-sources.mjs"),"utf8");
for(const marker of ["scanGitHubProject","parseGitHubRepoUrl","kk-project-github-scan/v1"]) assert.ok(projectSources.includes(marker),`project source marker missing: ${marker}`);
const projectFit=fs.readFileSync(path.join(CONFIG.packageRoot,"src","no-code","project-fit.mjs"),"utf8");
for(const marker of ["analyzeProjectContext","sourceIntelligence","websiteContent","packageRoot","integrationSupportFiles","newProjectSupportFiles","kk-project-fit/v1","APPLY-WINDOWS.ps1","APPLY-MAC.command"]) assert.ok(projectFit.includes(marker),`project-fit marker missing: ${marker}`);
...
for(const marker of ["familyFilters","selectFiltered","Essential design","Build the page yourself.","buildPageButton","downloadProjectButton","authToggle","authMode","Session cookie","projectFitTitle","projectFiles","projectFolder","githubRepoUrl","projectWebsiteUrl","scanGithubButton","scanWebsiteButton","certifyBuildButton","Analyze Project Fit"]) assert.ok(designExplorer.includes(marker),`Design Explorer marker missing: ${marker}`);
const browserless=fs.readFileSync(path.join(CONFIG.packageRoot,"src","reference-design","browserless.mjs"),"utf8");
for(const marker of ["setExactViewport","installNetworkGuard","kk-reference-design-evidence/v2","kk-reference-design-inspection/v2","familyCounts","establishReferenceSession","performFormLogin","inspectProjectWebsite","certifyGeneratedPreview","rawPixelSimilarityPct","structuralSimilarityPct"]) assert.ok(browserless.includes(marker),`browserless hardening marker missing: ${marker}`);
...
console.log("KK-FRONTEND v0.9.0 selftest PASS");
