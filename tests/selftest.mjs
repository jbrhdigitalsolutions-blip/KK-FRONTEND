...
assert.ok(fs.existsSync(path.join(CONFIG.packageRoot,"src","storage","r2-transport.mjs")));
...
for(const marker of ["familyFilters","selectFiltered","Essential design","Build the page yourself.","buildPageButton","downloadProjectButton","authToggle","authMode","Session cookie","projectFitTitle","projectFiles","projectFolder","githubRepoUrl","projectWebsiteUrl","scanGithubButton","scanWebsiteButton","referenceIntentPanel","useCollectionButton","fitTargetMapping","certifyBuildButton","Analyze Project Fit"]) assert.ok(designExplorer.includes(marker),`Design Explorer marker missing: ${marker}`);
const browserless=fs.readFileSync(path.join(CONFIG.packageRoot,"src","reference-design","browserless.mjs"),"utf8");
for(const marker of ["setExactViewport","installNetworkGuard","kk-reference-design-evidence/v2","kk-reference-design-inspection/v2","familyCounts","detectReferenceIntent","requiresChoice","establishReferenceSession","performFormLogin","inspectProjectWebsite","certifyGeneratedPreview","rawPixelSimilarityPct","structuralSimilarityPct"]) assert.ok(browserless.includes(marker),`browserless hardening marker missing: ${marker}`);
...
console.log("KK-FRONTEND v0.9.5 selftest PASS");
